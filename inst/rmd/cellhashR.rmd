...
optionalVars <- c('barcodeWhitelist', 'cellbarcodeWhitelist', 'minCountPerCell', 'metricsFile', 'rawCountsExport', 'molInfoFile')
...
if (!is.null(molInfoFile)) {
  saturationData <- CalculateSaturationFor10x(molInfoFile)
}
...
if (nrow(barcodeData) > 0 && ncol(barcodeData) > 0){
	...
	df <- GenerateCellHashingCalls(barcodeMatrix = barcodeData, methods = methods, cellbarcodeWhitelist = cellbarcodeWhitelist, metricsFile = metricsFile, perCellSaturation = saturationData)
