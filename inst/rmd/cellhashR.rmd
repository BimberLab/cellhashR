...
optionalVars <- c('barcodeWhitelist', 'cellbarcodeWhitelist', 'minCountPerCell', 'metricsFile', 'rawCountsExport', 'molInfoFile', 'h5File', 'methodsForConsensus', 'majorityConsensusThreshold')
...
if (nrow(barcodeData) > 0 && ncol(barcodeData) > 0){
	...
	df <- GenerateCellHashingCalls(barcodeMatrix = barcodeData, methods = methods, methodsForConsensus = methodsForConsensus, cellbarcodeWhitelist = cellbarcodeWhitelist, metricsFile = metricsFile, perCellSaturation = saturationData, demuxem.rawFeatureMatrixH5 = h5File, majorityConsensusThreshold = majorityConsensusThreshold)
...
if (!is.null(df) && sum(df$consensuscall == 'Discordant') > 0) {
  ...
  colNames <- if(is.null(methodsForConsensus)) methods else methodsForConsensus
  discord <- discord %>% group_by_at(colNames) %>% summarize(Total = n()) %>% arrange(desc(Total))
  discord$Percent <- discord$Total / sum(discord$Total)

  print(knitr::kable(discord, caption = 'Consensus Methods'))

  if (length(colNames) != length(methods)) {
    discord <- df[df$consensuscall == 'Discordant',]
    discord <- discord %>% group_by_at(methods) %>% summarize(Total = n()) %>% arrange(desc(Total))
    discord$Percent <- discord$Total / sum(discord$Total)

    print(knitr::kable(discord, caption = 'All Methods'))
  }
}
