...
getCountCutoff <- function(data, label, barcodeBlocklist = NULL) {
  ...
  if ((cutoff == max(data)) | (length(data[data > cutoff]) < 5)) {
...
GenerateCellHashCallsBFF <- function(barcodeMatrix, assay = "HTO", min_average_reads = 10, verbose = TRUE, methodName = 'bff', optimize_cutoffs, recover, doublet_thresh, neg_thresh, rec_meth){
  ...
  tryCatch({
    ...
    seuratObj <- BFFDemux(seuratObj = seuratObj, assay = assay, optimize_cutoffs = optimize_cutoffs, recover=recover, doublet_thresh=doublet_thresh, neg_thresh=neg_thresh, rec_meth=rec_meth)
    ...
    #   return(df)
      ...
      return(df)
    } else if (optimize_cutoffs==FALSE && recover==TRUE && rec_meth == 3){
...
BFFDemux <- function(seuratObj, assay, optimize_cutoffs, recover, doublet_thresh, neg_thresh, rec_meth) {
  barcodeMatrix <- GetAssayData(
    object = seuratObj,
    assay = assay,
    slot = 'counts'
  ...
  if (optimize_cutoffs == FALSE) {
    if (recover == FALSE) {
      ...
      if (rec_meth==1) {
        ...
        seuratObj <- .AssignCallsToMatrix(seuratObj, as.matrix(discrete), suffix = 'bff_rec', assay = assay)
        return(seuratObj)
      } else if (rec_meth==2){
